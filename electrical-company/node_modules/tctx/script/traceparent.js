"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.is_randomed = exports.is_sampled = exports.unsample = exports.sample = exports.parse = exports.make = exports.FLAG_RANDOM = exports.FLAG_SAMPLE = void 0;
const csprng_1 = require("@lukeed/csprng");
const W3C_TRACEPARENT_VERSION = '00';
/** The bitmask represenging a sampled traceparent */
exports.FLAG_SAMPLE = 0b00000001;
/** The bitmask representing a random traceparent */
exports.FLAG_RANDOM = 0b00000010;
function traceparent(version, trace_id, parent_id, flags) {
    return {
        version,
        trace_id,
        parent_id,
        flags,
        child() {
            return traceparent(this.version, this.trace_id, random(8), this.flags);
        },
        toString() {
            let flags = this.flags.toString(16).padStart(2, '0');
            return `${this.version}-${this.trace_id}-${this.parent_id}-${flags}`;
        },
    };
}
/**
 * Makes a new Traceparent which one can then `toString()` to get the value.
 *
 * By default the flags are both {@link FLAG_SAMPLE|sampled} and {@link FLAG_RANDOM|randomed}.
 *
 * @example
 *
 * ```js
 * const id = make();
 * String(id); // 00-aa3ee2e08eb134a292fb799969f2de62-62994ea4677bc463-01
 * const child = id.child();
 * String(child); // 00-aa3ee2e08eb134a292fb799969f2de62-5402ac6f6874d505-01
 * ```
 */
function make() {
    let id = random(24);
    return traceparent(W3C_TRACEPARENT_VERSION, id.slice(0, 32), id.slice(32), exports.FLAG_SAMPLE | exports.FLAG_RANDOM);
}
exports.make = make;
/**
 * Allows you to parse an incoming value into the areas, easy for a server to continue the trace chain.
 *
 * @example
 * ```js
 * const parent = parse(req.headers.traceparent); // 00-aa3ee2e08eb134a292fb799969f2de62-62994ea4677bc463-00
 * const child = parent.child();
 * String(child); // 00-aa3ee2e08eb134a292fb799969f2de62-5402ac6f6874d505-01
 * ```
 */
function parse(value) {
    value = value.trim();
    if (value.length < 55)
        return null;
    if (~value.indexOf('_'))
        return null; // regex, \W is [0-9a-fA-F_] — with underscore as well, which we dont want
    let segs = value.split('-');
    if (/\D/.test(segs[3]))
        return null;
    let v = segs[0], t = segs[1], p = segs[2], f = segs[3];
    if (v.length > 2 || /\W/.test(v))
        return null;
    if (v === W3C_TRACEPARENT_VERSION && segs.length > 4)
        return null; // we know we are on version 00, so we should only have 4 segments
    if (v === 'ff')
        return null;
    else
        v = W3C_TRACEPARENT_VERSION; // we'll etiher parse as V0, or make it V0
    if (t.length !== 32 || /\W/.test(t) || !/[^0]/.test(t))
        t = null;
    if (p.length !== 16 || /\W/.test(p) || !/[^0]/.test(p))
        t = p = null;
    if (f.length !== 2)
        return null;
    else
        f = parseInt(segs[3], 16);
    if (t == null || p == null) {
        let id = random(24);
        t ||= id.slice(0, 32);
        p ||= id.slice(32);
    }
    return traceparent(v, t, p, f);
}
exports.parse = parse;
// -- Utils
/**
 * Modifies the flags of a Traceparent to sample the traceparent flag bit.
 *
 * > [!NOTE]
 * > You may wish to `.child()` before you sample, as is required by the {@link https://www.w3.org/TR/trace-context-2/#sampled-flag|spec}
 */
function sample(id) {
    id.flags |= exports.FLAG_SAMPLE;
}
exports.sample = sample;
/**
 * Modifies the flags of a Traceparent to unsample the traceparent flag bit.
 *
 * > [!NOTE]
 * > You may wish to `.child()` before you unsample, as is required by the {@link https://www.w3.org/TR/trace-context-2/#sampled-flag|spec}
 */
function unsample(id) {
    id.flags &= ~exports.FLAG_SAMPLE;
}
exports.unsample = unsample;
/** Returns if the traceparent is currently being sampled. */
function is_sampled(id) {
    return (id.flags & exports.FLAG_SAMPLE) == exports.FLAG_SAMPLE;
}
exports.is_sampled = is_sampled;
/** Returns if the traceparent is currently random. */
function is_randomed(id) {
    return (id.flags & exports.FLAG_RANDOM) == exports.FLAG_RANDOM;
}
exports.is_randomed = is_randomed;
let IDX = 256, HEX = [];
for (; IDX--;)
    HEX[IDX] = (IDX + 256).toString(16).substring(1);
function random(size) {
    let a = (0, csprng_1.random)(size), i = 0, o = '';
    for (; i < a.length; i++)
        o += HEX[a[i]];
    return o;
}
